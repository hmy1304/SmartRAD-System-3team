...
public class ApprovalService {
    ...
    public ApprovalInboxResponse getApprovedApprovals(Long approverId) {
        ...
        // 1. Leave Applications (status = "승인완료" or "반려")
        List<LeaveApplication> approvedLeaves = leaveApplicationRepository.findAll().stream()
        ...
        // 2. Appointments (status = "COMPLETED" or "REJECTED")
        List<Appointment> approvedAppointments = appointmentRepository.findAll().stream()
        ...
        // Sort docs by requestedAt descending
        docDtos.sort((a, b) -> b.getRequestedAt().compareTo(a.getRequestedAt()));
